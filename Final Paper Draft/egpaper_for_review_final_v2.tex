...
For each class, we divide the recall values into 11 points ranging from 0 to 1 with step-size 0.1 plot that against maximum precision from that recall value upward. We obtain mAP by averaging over each classes AP \ref{mAP}\\ 

\begin{align}\label{mAP}
AP=\dfrac{1}{11}\sum_{r\in{0.0,...1.0}}AP_r \\*
=\dfrac{1}{11}\sum_{r\in{0.0,...1.0}}p_{interp}{r}
\end{align}
where:
\begin{equation*}
...
Each cell then determines its best candidate for predicting the bounding box of the object. This loss function includes the complete IoU (cite) loss as the box loss, the standard binary cross entropy for multi-label classification as the classification loss (allowing each cell to predict more than 1 class), and the novel distribution focal loss in the 3rd term (cite).
