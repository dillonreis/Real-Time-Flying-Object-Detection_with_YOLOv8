...
\mathcal{L}(\theta) = \lambda_{box}\mathcal{L}_{box}(\theta) + \lambda_{cls}\mathcal{L}_{cls}(\theta) + \lambda_{dfl}\mathcal{L}_{dfl}(\theta) + \phi\Vert \theta \Vert_2^2										
\end{equation}
...
Where \ref{Generalized Loss} is the generalized loss function incorporating the individual loss weights as well as weight decay $\phi$, \ref{Velocity} is the velocity term which takes momentum $\beta$ as a parameter, and \ref{Weight Update} which is the weight update term.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{figures/PR_curve.png}
    \caption{YOLOv8m PR Curve}
    \label{fig:PRCurve}
\end{figure}
